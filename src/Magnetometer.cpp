...
void Magnetometer::ProcessData(SensorBaseData *data)
{
	...
	property_get("ro.stmicro.magn.rot_matrix", rot_matrix, TOSTRING(CONFIG_ST_HAL_MAGN_ROT_MATRIX));
	sscanf(rot_matrix, "%d,%d,%d,%d,%d,%d,%d,%d,%d", &m[0], &m[1], &m[2], &m[3], &m[4], &m[5], &m[6], &m[7], &m[8]);
	data->raw[0] = GAUSS_TO_UTESLA(SENSOR_X_DATA(tmp_raw_data[0], tmp_raw_data[1], tmp_raw_data[2], m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8]));
	data->raw[1] = GAUSS_TO_UTESLA(SENSOR_Y_DATA(tmp_raw_data[0], tmp_raw_data[1], tmp_raw_data[2], m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8]));
	data->raw[2] = GAUSS_TO_UTESLA(SENSOR_Z_DATA(tmp_raw_data[0], tmp_raw_data[1], tmp_raw_data[2], m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8]));
